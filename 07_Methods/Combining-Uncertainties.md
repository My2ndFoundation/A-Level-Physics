...
```mermaid
flowchart TD
    A[Two measured quantities a and b<br/>with uncertainties Δa and Δb] --> B{How do they combine<br/>in the formula?}
    B -->|"Add or subtract: y = a ± b"| C["Add ABSOLUTE uncertainties<br/>Δy = Δa + Δb"]
    B -->|"Multiply or divide: y = ab or a/b"| D["Add PERCENTAGE uncertainties<br/>%y = %a + %b"]
    B -->|"Power: y = aⁿ"| E["Multiply % uncertainty by |n|<br/>%y = |n| × %a"]
    B -->|"Exact constant: y = ka"| F["% uncertainty unchanged<br/>Δy = k × Δa"]
    C --> G[Convert final % back to absolute<br/>Δy = (%y/100) × y]
    D --> G
    E --> G
    F --> G
```
