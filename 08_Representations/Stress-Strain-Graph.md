...
- x-axis: strain `ε` = extension ÷ original length — dimensionless (often a percentage).
- y-axis: stress `σ` = force ÷ cross-sectional area, in pascals (Pa).
...
- **Gradient** of the straight elastic region = the [[Young-Modulus]] `E = σ / ε` (Pa). A steeper line means a stiffer material. Use [[Finding-Gradient-from-a-Graph]] over the linear part only.
...
- From: a [[Force-Extension-Graph]] using `σ = F/A` and `ε = x/L`.
