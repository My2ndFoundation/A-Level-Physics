...
## [2026-05-19] lint | whole wiki (frontmatter focus)

- Audited: 401 content pages. Frontmatter health clean — 0 missing/no-frontmatter, 0 tag-format issues (post 20-MOC fix held), 0 empty `type:`, 0 type↔folder mismatches. 17 "missing key" flags were all template-conformant (worked-example has no `aliases`; source-template uses its own schema) — not defects per §4. Orphans: 0. Broken links: 0 untracked (2 maths-stub targets already registered in `.state/unresolved-links.md`).
- Renamed (CLAUDE.md §5.5 — broad-topic names + duplicate `Energy.md` basename ambiguity; user-confirmed layer-suffix scheme): `02_Foundations/GCSE-Foundations/Energy.md` → `Energy-GCSE.md`; `02_Foundations/GCSE-Foundations/Waves.md` → `Waves-GCSE.md`; `03_Physical-Quantities/Energy.md` → `Energy-Quantity.md` (via `git mv`, history preserved).
- Updated: 108 wikilinks repointed across 79 files (73→Energy-Quantity [A-Level-quantity default], 19→Waves-GCSE, 16→Energy-GCSE [foundation/GCSE-context: Foundations/GCSE-Foundations MOCs, Everyday-Physics, GCSE siblings]). 2 manual overrides for cross-layer self-refs (Energy-Quantity↔Energy-GCSE). Frontmatter aliases added: `Energy`→Energy-Quantity, `Waves`→Waves-GCSE, `GCSE energy`→Energy-GCSE. `.state/aliases.tsv` appended (bare `Energy`/`Waves` → new canonicals). Hardened earlier: `_meta/tags.md` (frontmatter tag-format rule), `_meta/templates/moc-template.md` (multi-tag demo), `physics-build-moc/SKILL.md` (tag-emit guard).
- Deferred: 10 non-MOC `status: draft` pages (07_Methods ×4, 10_Applications ×6 — maths/CS cross-subject stubs, by-design shallow). §5.5↔§6 rule tension resolved by disambiguation rather than a CLAUDE.md carve-out.
- Notes: link display text preserved via `[[Canonical|Original]]` so reading is unchanged. No content overwritten, no page deleted, no `raw/`/`_meta/` content edits beyond the 3 hardening files. No git commit (§5.11).

## [2026-05-20] expand | SI base unit definition pages
- Created: 04_Concepts/The-Metre.md, The-Kilogram.md, The-Second.md, The-Ampere.md, The-Kelvin.md (type: concept, status: usable) — 5 new pages, each with a `## Historical Development` section (~600–900 words) tracing the unit from its first definition (meridian arc / litre of water / solar day / silver electrolysis / Celsius scale) through artefact-based forms to the 2019 SI redefinition (fixed c / h / ΔνCs / e / kB). Each page includes a Mermaid timeline diagram + 1–3 Wikimedia Commons photos.
- Updated: 04_Concepts/SI-Units.md (base-unit table now links to all 5 definition pages); index.md (+5 Concepts entries); _meta/workflows/fetch_wikipedia_images_postpass.py (added overrides for the 5 unit pages so Wikipedia article search picks "Metre" / "Second" etc. instead of unrelated album titles).
- Deferred: none. Five other 04_Concepts pages were re-fetched by the postpass run (entries already in OVERRIDES) — their image sets refreshed but content unchanged.
- Notes: Initial main-script run mis-matched The-Metre → Powderfinger album and The-Second → Steppenwolf album; postpass overrides fixed both. Image-fetcher dedupe bug produced duplicate blocks on three pages — removed manually and captions improved from generic "from Wikipedia article X" to topic-specific ones. One off-topic Wikipedia image (Aleutian Islands International Date Line on The-Second) removed. No raw/ touched. No copyrighted text reproduced — all explanatory content authored from BIPM SI Brochure 9th edition + Wikipedia navigation only. No git commit (§5.11).

## [2026-06-27] ingest | AQA Physics 7407/7408 Specification (2015)
- Source: created 13_Sources/AQA-Specifications/AQA-Physics-7407-7408-Specification.md
- Dual-tagged: 277 existing OCR a-level pages received `aqa-7407-7408` (overlap kept, not deleted); Medical-Imaging also tagged.
- Created (77 atomic pages):
  - Particle physics (§3.2): Specific-Charge, Strong-Nuclear-Force, Fundamental-Interactions, Exchange-Particles, Annihilation-and-Pair-Production, Classification-of-Particles, Baryon-Number, Lepton-Number, Strangeness, Feynman-Diagram, Applying-Conservation-Laws-to-Particle-Interactions
  - Mandatory gaps: Nuclear-Radius (§3.8), Alternating-Current (§3.7.5.5)
  - Astrophysics (§3.9): Astronomical-Telescope, Reflecting-Telescope, Non-Optical-Telescopes, Resolving-Power, Rayleigh-Criterion, Apparent-Magnitude, Absolute-Magnitude, Magnitude-Distance-Equation, Black-Body-Radiation, Stellar-Spectral-Classes, Supernovae-Neutron-Stars-and-Black-Holes, Schwarzschild-Radius, Standard-Candles, Quasars, Exoplanet-Detection
  - Medical physics (§3.10): Lens-Equation, Lens-Power, Physics-of-the-Eye, Defects-of-Vision, Physics-of-the-Ear, Sound-Intensity-Level, X-ray-Attenuation, Effective-Half-Life, Radionuclide-Imaging, Gamma-Camera, Radiation-Therapy
  - Engineering physics (§3.11): Moment-of-Inertia, Rotational-Kinetic-Energy, Rotational-Motion, Torque-and-Angular-Acceleration, Angular-Momentum, Conservation-of-Angular-Momentum, Flywheels, Solving-Rotational-Dynamics-Problems, First-Law-of-Thermodynamics, Thermodynamic-Processes, pV-Diagram, Engine-Cycles, Second-Law-of-Thermodynamics, Reversed-Heat-Engines
  - Turning points (§3.12): Cathode-Rays, Thermionic-Emission, Millikan-Oil-Drop-Experiment, Maxwell-Speed-of-Light, Electron-Microscopes, Michelson-Morley-Experiment, Special-Relativity, Time-Dilation, Length-Contraction, Relativistic-Mass-and-Energy
  - Electronics (§3.13): Semiconductor-Devices, Hall-Effect-Sensor, Analogue-and-Digital-Signals, LC-Resonance-Filter, Ideal-Operational-Amplifier, Operational-Amplifier-Circuits, Combinational-and-Sequential-Logic, Logic-Gate-Diagram, Communication-Systems
  - Required practicals (§3.5/3.6/3.7/3.8): Determining-Resistivity-of-a-Wire, Investigating-Boyle-and-Charles-Laws, Investigating-Force-on-a-Current-Carrying-Wire, Investigating-Flux-Linkage-with-a-Search-Coil, Investigating-Inverse-Square-Law-for-Gamma-Radiation
